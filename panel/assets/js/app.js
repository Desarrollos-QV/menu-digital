    ...
    setup() {
        ...
        const toggleAddonOption = (group, option) => {
            ...
            if (existingIdx !== -1) {
                ...
                if (group.maxOptions > 1) {
                    ...
                    // Lógica Radio (Ãsnico: reemplaza otros del mismo grupo)
        ...
        const confirmShare = () => {
            ...
            if (shareContext.value === 'ticket') {
                ...
                msg = `¡Hola *${ord.customerId ? ord.customerId.name : (ord.customerName || 'Cliente')}*! Aquí tienes el detalle de tu pedido en *${settings.settings.value.appName || 'Tengo Hambre'}*.âo.\n\n`;
                msg += `ðY"¦ *Folio:* #${ord._id.slice(-6).toUpperCase()}\n`;
                msg += `ðY". *Fecha:* ${new Date(ord.createdAt).toLocaleString()}\n`;
                msg += `ðY'³ *Método de pago:* ${ord.paymentMethod}\n${deliverySection}\n\n`;
                msg += `ðY>' *Productos:*\n${items}\n\n`;
                msg += `ðY'° *SubTotal:* $${(ord.subtotal || 0).toFixed(2)}${commLine}${deliveryCostLine}\n`;
                msg += `â­ *TOTAL: $${ord.total.toFixed(2)}*\n\n`;
                msg += `ðYT ¡Gracias por tu preferencia!`;
            } else {
