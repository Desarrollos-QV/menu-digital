...
createApp({
    setup() {
        ...
        // --- L�"GICA LOCAL PARA MODAL DETALLE ITEM ---
        ...
        const toggleAddonOption = (group, option) => {
            ...
            if (existingIdx !== -1) {
                ...
                if (group.maxOptions > 1) {
                    ...
                    // Lógica Radio (�snico: reemplaza otros del mismo grupo)
        ...
        // --- L�"GICA IMPRESI�"N DE TICKETS PRO ---
        ...
        const generatePDF = () => {
            ...
            // ---- DATOS DE ENV�O (solo si es a domicilio) ----
        ...
        const confirmShare = () => {
            ...
            if (shareContext.value === 'ticket') {
                ...
                msg = `¡Hola *${ord.customerId ? ord.customerId.name : (ord.customerName || 'Cliente')}*! Aquí tienes el detalle de tu pedido en *${settings.settings.value.appName || 'Tengo Hambre'}*.�o.\n\n`;
                msg += `�Y"� *Folio:* #${ord._id.slice(-6).toUpperCase()}\n`;
                msg += `�Y". *Fecha:* ${new Date(ord.createdAt).toLocaleString()}\n`;
                msg += `�Y'� *Método de pago:* ${ord.paymentMethod}\n${deliverySection}\n\n`;
                msg += `�Y>' *Productos:*\n${items}\n\n`;
                msg += `�Y'� *SubTotal:* $${(ord.subtotal || 0).toFixed(2)}${commLine}${deliveryCostLine}\n`;
                msg += `� *TOTAL: $${ord.total.toFixed(2)}*\n\n`;
                msg += `�YT ¡Gracias por tu preferencia!`;
            } else {
        ...
        // --- FUNCI�"N: PROCESAR VENTA Y ABRIR TICKET ---
        ...
        // 4. MANEJAR BOT�"N "ATR�S" DEL NAVEGADOR
